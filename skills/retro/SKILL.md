...
- **Approved fact** → fix at the fact's single source per Placement by loading (owning doc, e.g. README, or the entry file); diff first; only the fact, never the harness block.
