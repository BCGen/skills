...
An approved fix SHALL be applied in place, to the skill the audit found it in. There is no
delegate skill for this — writing and editing a SKILL.md is native to the model, and the
audit already holds the evidence the edit rests on.

An unapproved finding SHALL remain a proposal: the report includes the exact edit for the
user to apply or decline.

#### Scenario: An approved fix

- **WHEN** the user approves a fix the audit proposed
- **THEN** skill-auditing applies it to that skill in place, and says what it changed

#### Scenario: A declined fix

- **WHEN** the user declines a proposed fix
- **THEN** the exact edit stays in the report and nothing is written
...
Findings SHALL be proposed, never applied without consent — the audit diagnoses, and applies
