...
- 2026-07-06 · task: root cause of the silent check failure · evidence: "Failed to check for deleted skills" is an upstream bug — the deletion check passes the lock's shorthand source ("BCGen/skills") straight to cloneRepo, which git-clones the raw string (no shorthand→URL resolution, unlike the per-skill update path), so the check throws for every owner/repo-installed skill; had it worked, the rename would have prompted removal of the old name. Falsified "our fault": the identical failure reproduces with vercel-labs' own agent-skills collection, and `git clone <shorthand>` fatals directly
